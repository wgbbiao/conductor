...
**Conductor** 是一个 AI 原生的研发流程编排平台。它不是把 AI 简单"外挂"到传统工具上，而是把 **AI 当作第一公民**：可插拔的编码 Agent（Codex、Claude Code 等）、可挂载的 skills 生态（OpenSpec、Superpowers、gstack 等），与多个角色（PM、开发、测试、设计、Reviewer）在一套连贯、可观测、可治理的流程中协同——从需求一路贯通到发布。Conductor 像指挥家（Conductor）一样，调度 AI 工具、技能与角色，奏出完整的研发流程。

换言之，它是一个以 **AI 为第一公民**的软件开发流程管理平台。
...
- 📜 **过程可观测、可治理** —— AI 的每一步操作、每一次人机交接都有记录，可审计。支持[时间线回放](./docs/design/replay.md)（只读重看一个工作项的完整事件流）；更强的"从某点重新执行"能力在规划中。
...
> 同步到 GitHub 仓库的 Topics（在仓库 **About ⚙️ → Topics** 中设置）。

`ai-native` · `ai-agents` · `llm` · `ai-coding-assistant` · `sdlc` · `orchestration` · `workflow` · `developer-tools` · `developer-productivity` · `collaboration` · `project-management` · `devops` · `automation` · `claude-code` · `codex`
