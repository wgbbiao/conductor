...
> 关联：[ROADMAP](../../ROADMAP.md) · [Phase 1 架构](./phase-01-architecture.md) · [Phase 1 计划 v1.1](../plans/2026-07-09-phase-01-backend-core.md) · [回放设计](./replay.md)
>
...
| B10 | **回放/时间旅行** | ❓→已设计 | P1(R1)+ | B5 | 已定义三档语义（[replay.md](./replay.md)）：R1 时间线只读回放（低成本，事件已具备）、R2 导出/重建（中）、R3 可重放执行（高，需 1:N 执行历史）。**建议先做 R1，承诺措辞对齐到 R1** |
...
README/ROADMAP 宣传"可回放"，事件账本（`AuditEvent`/`ToolEvent` 带 `seq`）也为此设计，此前**没有"回放"功能模块**。现已定义三档语义（见 [replay.md](./replay.md)）：

- **R1 时间线只读回放**（低成本，事件账本已具备）——**P1 跑通后优先做**，README 措辞对齐到 R1。
- **R2 事件流导出/重建**（中）——中期可观测性增强。
- **R3 可重放执行/时间旅行**（高，需 WorkItem:WorkflowRun = 1:N + 状态快照）——不承诺时间表，若要做需单独 ADR。
...
| **P5** | A7,B9,B10(R2/R3),C1 | 全流程贯通、Git/CI、回放增强、扩展市场。注：**回放 R1 随 P1 后做** |
